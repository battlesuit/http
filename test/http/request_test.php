...
class RequestTest extends MessageTester {  
  ...
  function test_method_lowercasion() {
    $r = new Request(null, 'POST');
    $this->assert_eq($r->method(), 'post');
  }
  
  function test_with_path() {
    $r = new Request('http://domain.de/foo/bar');
    $this->assert_eq($r->path(), '/foo/bar');
  }
  
  function test_with_path_including_file() {
    $r = new Request('http://domain.de/foo/bar/index.php/asf');
    
    $this->assert_eq($r->path(), '/foo/bar');
  }
  
  function test_with_path_and_query_string() {
    $r = new Request('http://domain.de/foo/bar?param=hallo');
    $this->assert_eq($r->path(), '/foo/bar');
  }
  
  function test_with_path_and_file() {
    $r = new Request('http://domain.de/foo/bar/index.php?param=hallo');
    $this->assert_eq($r->path(), '/foo/bar');
  }
  
  function test_script_url() {
    $r = new Request('http://domain.de/foo/bar/index.php/bla?param=hallo');
    $this->assert_eq($r->script_url(), 'http://domain.de/foo/bar/index.php');
  }
  
  function test_host_url() {
    $r = new Request('http://domain.de/foo/bar/index.php/bla?param=hallo#fragment');
    $this->assert_eq($r->host_url(), 'http://domain.de');
  }
  
  function test_host_url_with_port() {
    $r = new Request('http://domain.de:80/foo/bar/index.php/bla?param=hallo#fragment');
    $this->assert_eq($r->host_url(true), 'http://domain.de:80');
  }
  
  function test_path_info() {
    $r = new Request('http://domain.de/foo/bar/index.php/foo/bar?param=hallo');
    $this->assert_eq($r->path_info(), '/foo/bar');
  }
  
  function test_path_info_with_dotted_dir() {
    $r = new Request('http://domain.de/foo.mooh/bar.de/index.php/foo/bar?param=hallo');
    $this->assert_eq($r->path_info(), '/foo/bar');
  }
  
  function test_path_info_without_scriptfile() {
    $r = new Request('http://domain.de/foo.mooh/bar.de/foo/bar?param=hallo');
    $this->assert_eq($r->path_info(), '');
    $this->assert_eq($r->path(), '/foo.mooh/bar.de/foo/bar');
  }
  
  function test_method() {
    $r = new Request(null, 'delete');
    $this->assert_equality($r->method(), 'delete');
  }
  
  function test_method_by_query() {
    $request = new Request('http://domain.de?_method=put', 'post');
    $this->assert_equality($request->method(), 'put');
  }
  
  function test_method_by_data() {
    $r = new Request('http://domain.de', 'post', array('_method' => 'delete'));
    $this->assert_equality($r->method(), 'delete');
  }
  
  function test_url() {
    $request = new Request('http://domain.de/hello/world?destroy=true', 'post');
  ...
  function test_url_components() {
    $request = new Request('http://tom:552@domain.de:80/hello/world/call.php/path/info?id=12&name=mel#my-frag', 'post');
  ...
  function test_resource_path() {
    $request = new Request('http://domain.de:80/hello/world', 'get');
    $this->assert_eq($request->resource_path(), '/hello/world');
    
    $request = new Request('http://domain.de:80/hello/world/index.php/foo/bar', 'get');
    $this->assert_eq($request->resource_path(), '/foo/bar');
  }
  
  function test_content_length_default() {
    $request = new Request('http://domain.de:80/hello/world/call.php/path/info?id=12&name=mel', 'post', array('foo' => 'bar'));
  ...
  function test_via_put() {
    $r = new Request(null, 'put');
    $this->assert_true($r->via_put());
  }
  
  function test_accepted_mimes() {
    $request = new Request('http://domain.de:80', 'post', array(), array('accept' => 'text/xml,text/plain,application/*,*/javascript'));
  ...
  function test_to_array() {
    $request = new Request('http://tom:552@domain.de:80/hello/world/call.php/path/info?id=12&name=mel#my-frag', 'post', array('simon' => 'says'), array('accept' => 'text/xml,text/plain,application/*,*/javascript'));
